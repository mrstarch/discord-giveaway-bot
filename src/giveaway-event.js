...
async function createAndSendWinnerMessage() {
    ...
    let winnerUserName = (await client.users.fetch(winnerObj['userId'])).displayName;
...
function chooseHost() {
    let giveawayHostsData = JSON.parse(fs.readFileSync(giveawayHostsFile, 'utf8'));
    ...
    if(hostUsers.length - 1 == currentHostIndex) giveawayHostsData['currentHostIndex'] = 0;
    else giveawayHostsData['currentHostIndex'] += 1;

    fs.writeFileSync(giveawayHostsFile, JSON.stringify(giveawayHostsData), 'utf8');
