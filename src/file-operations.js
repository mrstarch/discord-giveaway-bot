...
function writeMapToJsonFile(mapObj, filePath){
    fs.writeFileSync(filePath, JSON.stringify(Array.from(mapObj.entries())), 'utf-8');
}
...
function checkVoiceUsersActivityFile() {
    if (!fs.existsSync(voiceUsersActivityFile) || (fs.existsSync(voiceUsersActivityFile) && fs.readFileSync(voiceUsersActivityFile) == "")) {
...
function checkRewardFile() {
    ...
    if (!fs.existsSync(rewardsFile) || (fs.existsSync(rewardsFile) && fs.readFileSync(rewardsFile) == "")) {
...
function checkGiveawayHostsFile() {
    ...
    if (!fs.existsSync(giveawayHostsFile) || (fs.existsSync(giveawayHostsFile) && fs.readFileSync(giveawayHostsFile) == "")) {
        fs.writeFileSync(giveawayHostsFile, JSON.stringify(defaultHostsJson), 'utf-8');
    }
}
